locals {
  ami_name_al2023rootless = "${var.ami_name_prefix_al2023}-hvm-2023.0.${var.ami_version_al2023}${var.kernel_version_al2023}-x86_64-rootless"
  default_tags = {
    os_version          = "Amazon Linux 2023"
    source_image_name   = "{{ .SourceAMIName }}"
    ecs_runtime_version = "Docker version ${var.docker_version_al2023} Rootless"
    ecs_agent_version   = "${var.ecs_agent_version}"
    ami_type            = "al2023rootless"
    ami_version         = "2023.0.${var.ami_version_al2023}"
  }
  merged_tags = merge("${local.default_tags}", "${var.tags}")
}

source "amazon-ebs" "al2023rootless" {
  ami_name            = "${local.ami_name_al2023rootless}"
  ami_description     = "Amazon Linux AMI 2023.0.${var.ami_version_al2023} Rootless x86_64 ECS HVM EBS"
...
build {
  ...
  provisioner "shell" {
    script = "scripts/al2023rootless/docker-rootless.sh"
  }
  ...
  provisioner "shell" {
    script = "scripts/al2023rootless/enable-services.sh"
  }
